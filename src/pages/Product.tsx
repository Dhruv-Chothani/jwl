...
export function ProductPage() {
  ...
  return (
    <SiteLayout>
      <Helmet>
        <title>{product.name} — Maison Lumière</title>
        <meta name="description" content={product.description} />
        <meta property="og:title" content={`${product.name} — Maison Lumière`} />
      ...
      <section className="mx-auto max-w-7xl px-6 py-16 md:px-10 md:py-24 grid gap-12 lg:grid-cols-2 lg:gap-20">
        ...
        <div className="flex flex-col justify-center">
          <Link to="/collections" className="eyebrow hover:text-gold">← {product.category}</Link>
          <h1 className="mt-6 font-display text-5xl md:text-6xl text-balance">{product.name}</h1>
          <p className="mt-6 text-2xl font-light tracking-wide">€{product.price.toLocaleString()}</p>
          <div className="my-10 h-px w-16 bg-gold" />
          <p className="text-foreground/75 leading-relaxed text-lg">{product.description}</p>
          ...
          <div className="mt-12 flex flex-wrap gap-4">
            ...
            <Link to="/contact" className="px-2 py-4 text-xs tracking-[0.3em] uppercase border-b border-foreground/40 hover:text-gold hover:border-gold">
