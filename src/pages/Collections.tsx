...
export function CollectionsPage() {
  ...
  return (
    <SiteLayout>
      <Helmet>
        <title>Collections — Maison Lumière</title>
        <meta name="description" content="Browse rings, earrings, bracelets and necklaces handcrafted in our Paris atelier." />
        <meta property="og:title" content="Collections — Maison Lumière" />
      </Helmet>
          ...
          {categories.map((c) => (
            <button
              ...
              className={`text-xs tracking-[0.3em] uppercase pb-2 border-b transition-colors ${
                filter === c ? "text-gold border-gold" : "text-foreground/60 border-transparent hover:text-foreground"
              }`}
            >
              {c}
            </button>
